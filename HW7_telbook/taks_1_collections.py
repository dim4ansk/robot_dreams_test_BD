...
def add():
    while True:
        ...
        if len(name) > 0 and number.isdigit() and len(number) == 10:
            if name in [i for i in dict_contact.keys()]:
                print("A contact with this name is already in your book!")
                break
            elif number in [i for i in dict_contact.values()]:
...
def info():
    ...
    if name_info in [i for i in dict_contact.keys()]:
...
def delete():
    ...
    if name_del in [i for i in dict_contact.keys()]:
...
while True:
    ...
    if choice.isdigit() and 6 > int(choice) > 0:
        ...
        print("You can enter only numbers whose value is from 1 to 5 include or 0 for exit. Try again!")
